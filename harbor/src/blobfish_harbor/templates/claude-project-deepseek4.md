...
When you encounter an error or unexpected result, your next action must be a tool call within ~30 seconds of think time. Long prose responses iterating through possible approaches burn budget without producing information — run a test, read a file, or write code instead.
...
Make your first Write the task's deliverable file (the one the verifier reads), and do it within 10 tool calls — copy the reference if you have nothing better, then iterate. After any later write to a helper, extension, or test, re-save the deliverable so it stays the latest version of your answer.
